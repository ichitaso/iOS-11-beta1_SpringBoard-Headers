//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Nov 15 2013 15:37:37).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "SBWorkspaceEntity.h"

@class SBApplicationSceneEntity;

@interface SBStarkWorkspaceNowPlayingEntity : SBWorkspaceEntity
{
    SBApplicationSceneEntity *_applicationSceneEntity;
    SBApplicationSceneEntity *_application;
}

+ (id)entityFromApplicationSceneEntity:(id)arg1;
@property(readonly, nonatomic) SBApplicationSceneEntity *application; // @synthesize application=_application;
- (void).cxx_destruct;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)descriptionBuilderWithMultilinePrefix:(id)arg1;
- (Class)viewControllerClass;
- (CDUnknownBlockType)_generator;
- (_Bool)_supportsLayoutRole:(long long)arg1;
- (id)applicationSceneEntity;
- (_Bool)isStarkNowPlayingEntity;
- (id)initWithIdentifier:(id)arg1 displayChangeSettings:(id)arg2;
- (id)initWithApplicationSceneEntity:(id)arg1;

@end

